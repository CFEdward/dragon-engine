...
using namespace dragon::graphics::d3d12::shaders;
...
public:
	...
	IDxcBlob* compile(shader_file_info info, std::filesystem::path full_path)
	{
		...
		LPCWSTR args[]
		{
			...
			L"-Qstrip_debug"				// String debug information into a separate blob
		};
...
bool compile_shaders()
{
	...
	// Compile shaders and put them all together in a buffer in the same order of compilation
	for (u32 i{ 0 }; i < engine_shader::count; ++i)
	{
		auto& info = shader_files[i];
