...
{
	auto script = dragon::script::registry().find(tag);
	assert(script != dragon::script::registry().end() && script->first == tag);
...
{
	const u32 size{ (u32)dragon::script::script_names().size() };
	if (!size) return nullptr;
	CComSafeArray<BSTR> names(size);
	for (u32 i{ 0 }; i < size; ++i)
	{
		names.SetAt(i, A2BSTR_EX(dragon::script::script_names()[i].c_str()), false);
	}
