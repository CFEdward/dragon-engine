	...
	explicit d3d12_command(ID3D12Device10* const device, D3D12_COMMAND_LIST_TYPE type)
	{
		...
		for (u32 i{ 0 }; i < frame_buffer_count; ++i)
		{
			...
			NAME_D3D12_OBJECT_Indexed(
				frame.cmd_allocator, i,
				type == D3D12_COMMAND_LIST_TYPE_DIRECT ? L"GFX Command Allocator" : type == D3D12_COMMAND_LIST_TYPE_COMPUTE ? L"Compute Command Allocator" : L"Command Allocator"
			);
		}
...
private:
	...
	struct command_frame
	{
		...
		void wait(HANDLE fence_event, ID3D12Fence1* fence)
		{
			...
			{
				// We have the fence create an event which is signalled once the fence's current value equals "fence_value"
...
bool initialize()
{
	...
	HRESULT hr{ S_OK };
	DXCall(CreateDXGIFactory2(0, IID_PPV_ARGS(&dxgi_factory)));
...
void create_a_root_signature()
{
	...
	{ // paramm 2: Descriptor table (unbounded/bindless)
		auto& param = params[2];
